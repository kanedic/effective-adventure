...
//						.map(url->url.getFile())
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
		 * movieFile에 들어가는 값은 null일 수도 있다.
		 ...
		 * movie 값으로 이미지 파일의 이름. 즉 movieFile 등이 파라미터로 넘어온다.
		 * 
...
//		File movieFile=Optional.ofNullable(req.getParameter("movie"))
		File movieFile=Optional.ofNullable(req.getParameter("movieOrImage"))
		...
		resp.setContentType(application.getMimeType(movieFile.getName()));
...
//		try(Closable 객체) {}catch(exception) {}finally {} try with resource 구문
		try(OutputStream os= resp.getOutputStream()){
			...
			Files.copy(movieFile.toPath(),os);
		}
