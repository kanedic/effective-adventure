...
@WebServlet("/AjaxCalenderGetMonth.do")
public class AjaxCalenderServlet2 extends HttpServlet{
	@Override
...
//		out.println(mix);
		...
		LocalDate firstDisplay = firstDOM.minusDays(offset); //9월 29일
		 ...
		 String json = Arrays.stream(Month.values())
		            .map(m -> String.format("{\"getMon\": \"%d\", \"mon\": \"%s\"}", m.getValue(), m.getDisplayName(java.time.format.TextStyle.FULL, locale)))
		            .collect(Collectors.joining(",", "[", "]"));  // JSON 배열 형식으로 만들기
